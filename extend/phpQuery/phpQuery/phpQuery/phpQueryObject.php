...
//			var_dump($documentID->getDocumentID());
		$id = $documentID instanceof self
			...
			: $documentID;
//		var_dump($id);
		if (! isset(\phpQuery\phpQuery::$documents[$id] )) {
//			var_dump(phpQuery::$documents);
			throw new Exception("Document with ID '{$id}' isn't loaded. Use phpQuery::newDocument(\$html) or phpQuery::newDocumentFile(\$file) first.");
		}
		$this->documentID = $id;
		$this->documentWrapper =& \phpQuery\phpQuery::$documents[$id];
...
//			print_r(array_slice(debug_backtrace(), 3));
	...
	protected function isChar($char) {
		return extension_loaded('mbstring') && \phpQuery\phpQuery::$mbstringSupport
			? mb_eregi('\w', $char)
			: preg_match('@\w@', $char);
	}
	/**
	...
	public function find($selectors, $context = null, $noHistory = false) {
		...
		foreach($queries as $selector) {
			...
			foreach($selector as $s) {
				// TAG
				$isTag = extension_loaded('mbstring') && \phpQuery\phpQuery::$mbstringSupport
					? mb_ereg_match('^[\w|\||-]+$', $s) || $s == '*'
					: preg_match('@^[\w|\||-]+$@', $s) || $s == '*';
				if ($isTag) {
					if ($this->isXML()) {
