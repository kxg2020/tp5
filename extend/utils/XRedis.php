...
class XRedis{
    ...
    protected static $redis_obj = null;//存储当前类的实例化变量

    protected $_obj = null; // 存储Redis原生类实例的变量
    ...
    public function __construct() {
        ...
        if(null == $this->_obj) {
            ...
            $this->_obj->connect('127.0.0.1',6379);
    ...
    public function set($key, $value, $timeOut=0) {
        ...
        $setRes =  $this->_obj->set($key, $value);

        if ($timeOut > 0) $this->_obj->expire($key, $timeOut);
    ...
    public  function get($key){
        ...
        $setRes =  $this->_obj->get($key);//不存在返回false
...
//			echo "$key - $value \n";
    ...
    public function lranges($key,$head,$tail){
        return $this->_obj->lrange($key,$head,$tail);
    }
    ...
    public function smembers($key){
        $re =   $this->_obj->exists($key);//存在返回1，不存在返回0
        if(!$re) return false;
        return $this->_obj->smembers($key);
    }
    ...
    public function zadd($key,$score_value,$timeOut =0){
        ...
        foreach($score_value as $score=>$value){
            $re =  $this->_obj->zadd($key,$score,$value);//当修改时，可以修改，但不返回更新数量
            $re && $a+=1;
            if ($timeOut > 0) $this->_obj->expire($key, $timeOut);
        }
        return $a;
    ...
    public function zrange($key,$min = 0 ,$num = 1,$order = 'desc'){
        ...
        if('desc' == strtolower($order)){
            $re = $this->_obj->zrevrange($key,$min ,$min+$num-1);
        }else{
            $re = $this->_obj->zrange($key,$min ,$min+$num-1);
        }
    ...
    public function zrank($key,$member,$type = 'desc'){
        ...
        if($type == 'desc'){
            $re = $this->_obj->zrevrank($key,$member);//其中有序集成员按score值递减(从大到小)顺序排列，返回其排位
        }else{
            $re = $this->_obj->zrank($key,$member);//其中有序集成员按score值递增(从小到大)顺序排列，返回其排位
        }
    ...
    public function zrangbyscore($key,$star,$end){
        return $this->_obj->ZRANGEBYSCORE($key,$star,$end);
    }
    ...
    function zscore($key,$member){
        return $this->_obj->ZSCORE($key,$member);
    }
    /*------------------------------------4.end sort set结构----------------------------------------------------*/
    ...
    public function hset_json($redis_key,$token,$id,$data,$timeOut = 0){
        ...
        $re = $this->_obj -> hSet($redis_table_name,$redis_key_name,$redis_info);//存入缓存
        if ($timeOut > 0) $this->_obj->expire($redis_table_name, $timeOut);//设置过期时间
        return $re;
    }
    ...
    public function hget_json($redis_key,$token,$id){
        $re =   $this->_obj->hexists($redis_key['key'].':'.$token,$redis_key['field'].':'.$id);//返回缓存中该hash类型的field是否存在
        if($re){
            $info = $this->_obj->hget($redis_key['key'].':'.$token,$redis_key['field'].':'.$id);
    ...
    public function hval($key){
        $re =   $this->_obj->exists($key);//存在返回1，不存在返回0
        if(!$re) return false;
        $vals = $this->_obj -> hvals($key);
        $keys = $this->_obj -> hkeys($key);
    ...
    public function incre($key,$num = 1,$member = '',$type=''){
        ...
        switch(strtolower(trim($type))){
            case "zset":
                $re = $this->_obj->zIncrBy($key,$num,$member);//增长权值
                break;
            case "hash":
                $re = $this->_obj->hincrby($key,$member,$num);//增长hashmap里的值
                break;
            default:
                if($num > 0){
                    $re = $this->_obj->incrby($key,$num);//默认增长
                }else{
                    $re = $this->_obj->decrBy($key,-$num);//默认增长
                }
                break;
        }
        if($re) return $re;
    ...
    function flush($type = 0){
        if($type) {
            $this->_obj->flushAll();//清除所有数据库
        }else{
            $this->_obj->flushdb();//清除当前数据库
        }
    }
    ...
    public function exists($keys,$type = '',$field=''){
        switch(strtolower(trim($type))){
            case 'hash':
                $re = $this->_obj->hexists($keys,$field);//有返回1，无返回0
                break;
            default:
                $re = $this->_obj->exists($keys);
                break;
        }
        return $re;
    }
    ...
    public function delete($key,$type,$field = ''){
        switch(strtolower(trim($type))){
            case 'hash':
                $re = $this->_obj->hDel($key,$field);//返回删除个数
                break;
            case 'set':
                $re = $this->_obj->sRem($key,$field);//返回删除个数
                break;
            case 'zset':
                $re = $this->_obj->zDelete($key,$field);//返回删除个数
                break;
            default:
                $re = $this->_obj->del($key);//返回删除个数
                break;
        }
        return $re;
    }
